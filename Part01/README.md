...
PS3.9之前指定了和ACR-NEMA2.0兼容的点对点通信的服务和协议。现在已不再使用。

#### <a name="ps3-10-media-storage-and-file-format-for-media-interchange"></a> PS3.10: 用于媒体交换的媒体存储和文件格式

[PS3.10](../Part10/README.md)指定了在可移动存储中使用的一种新的媒体影像信息存储模型（[图表6.10-1](#PS3.1_6.10-1)）。本部分的目的是提供一个框架，允许在各种物理存储介质上交换各种类型的医学图像和相关信息。

&emsp;&emsp;***提示***

&emsp;&emsp;&emsp;&emsp;可以通过[图表5-1](#PS3.1_5-1)来了解媒体交换模型和网络模型之间的关系。

[PS3.10](../Part10/README.md)指定了：

* 用于在存储介质上存储医学图像和相关信息的分层模型。 此模型引入了媒体存储应用程序配置文件的概念，该配置文件指定DICOM标准的应用程序特定子集，媒体存储实现可以声明符合该子集。 这种一致性仅适用于存储介质内容的写入，读取和更新
* 支持任何信息对象封装的DICOM文件格式
* 安全的DICOM文件格式，支持将DICOM文件格式封装在密码信封中
* DICOM文件服务，独立于基础媒体格式和物理媒体

[PS3.10](../Part10/README.md)定义了各种媒体存储概念：

* 在单个介质上标识一组文件的方法
* 在特定文件系统中命名DICOM文件的方法。

<p align="center"><img src="PS3.1_6.10-1.svg"/></p>
<p align="center"><a name="PS3.1_6.10-1"></a>图表6.10-1. DICOM媒体交换通信模型</p>
